...
async fn serves_health_and_host_info_on_a_unix_socket() {
    ...
    // vm_id is not tracked), and so is ThawGuest.
    let status = client
        .freeze_guest(pb::FreezeGuestRequest::default())
        .await
        .expect_err("FreezeGuest for an unknown VM must fail NOT_FOUND");
    ...
    let status = client
        .thaw_guest(pb::ThawGuestRequest {
            vm_id: "a".repeat(64),
        })
        .await
        .expect_err("ThawGuest for an unknown VM must fail NOT_FOUND");
    assert_eq!(status.code(), Code::NotFound);
    ...
    // A persistent Firecracker VM is refused outright. Persistence means a
    // per-VM systemd controller unit, and the Firecracker path here runs the
    // microVM in-process for the length of one request instead; there is no
    // unit for a persistent one to be adopted from or restarted through.
    let status = client
        .create_vm(pb::VmSpec {
            backend: pb::Backend::Firecracker as i32,
            persistent: true,
