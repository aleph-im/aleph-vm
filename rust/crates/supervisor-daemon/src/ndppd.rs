...
impl NdpProxy {
    ...
    fn update_conf(&self, state: &mut NdpState) {
        let contents = self.render(&state.ranges);
        if let Err(error) = self.edge.write_conf(&contents) {
            tracing::error!(error, "cannot update the ndppd configuration");
            return;
        }
    ...
    pub fn add_range(&self, interface: &str, address_range: &str, update_service: bool) {
    ...
    pub fn delete_range(&self, interface: &str, update_service: bool) {
        let mut state = self.lock();
...
mod tests {
    ...
    async fn the_config_matches_the_python_update_ndppd_conf() {
        ...
        proxy.add_range("vmtap3", "fc00:1:2:3::10/124", true);
        proxy.add_range("vmtap4", "fc00:1:2:3::20/124", true);
        ...
        proxy.delete_range("vmtap3", true);
    ...
    async fn priming_at_adoption_never_touches_the_file_or_service() {
        let (proxy, edge) = proxy();
        proxy.add_range("vmtap3", "fc00:1:2:3::10/124", false);
    ...
    async fn deleting_an_unknown_interface_is_a_no_op() {
        let (proxy, edge) = proxy();
        proxy.delete_range("vmtap9", true);
        assert!(edge.written().is_empty());
    }
}
