...
//!
//! Schema (the SQLAlchemy `PortMapping` model): `port_mappings(id INTEGER
...
//! row's values. The test fixture database was generated by the Python
//! daemon's SQLAlchemy models (frozen golden, see src/test_fixtures.rs).
//!
...
mod tests {
    ...
    fn the_legacy_migration_copies_active_rows_once() {
        // C2 parity: the fixture legacy DB was generated by the Python
        // daemon's SQLAlchemy models (frozen golden) with two active rows and
        // one soft-deleted row.
        let tmp = tempfile::tempdir().unwrap();
