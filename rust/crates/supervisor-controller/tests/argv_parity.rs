...
//! the since-removed `scripts/generate_controller_argv_fixtures.py` from the
//! actual Python QemuVM (the parity oracle), captured from the Python daemon's own models before that daemon was removed (2026-08); they are frozen goldens now, edit them only to record a deliberate change. A fixture is a JSON pair
