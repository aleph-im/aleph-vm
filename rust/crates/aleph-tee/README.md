...
Always available: the report and quote parsers, the `report_data` binding
schemes, the owner-auth envelope, and the X.509 extension.
...
# async fn run(raw_quote: &[u8]) -> anyhow::Result<()> {
let quote = parse_tdx_quote(raw_quote)?;
let now = SystemTime::now();
let collateral = fetch_collateral(&collateral_request(&quote.signature.pck_chain_pem)?, now).await?;
