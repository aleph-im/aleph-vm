...
- Conditional wiring: when no workload sidecar / no `content.workload` is present, the cmdline, disks, and measurement are byte-identical to today. `test_vm_snp`'s pinned measurement (`de933c37…`) must not change.
