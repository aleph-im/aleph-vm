...
2. **Migration (revised 2026-06-16 after review):** the provisional directory-based `export_vm`/`import_vm`/`get_migration_status` flow is **deleted** (unused, no live caller, redundant with the P2P path). Migration is the agent doing network transport + disk/overlay prep, then driving **standard lifecycle RPCs**: import builds a `CreateVmSpec` from the network-fetched message and calls `create_vm` (the no-download spec path adopts the already-staged overlay); failed-export restart uses `start_vm`; cleanup uses `delete_vm(wipe=False)`. One thin migration-specific method survives, `stop_vm_for_export`, because export requires a graceful guest powerdown (QMP `system_powerdown`, wait for halt) for a consistent exported overlay, which `stop_vm` (systemd stop) does not provide. Live/hypervisor-assisted migration is a future item, not needed today. The orphan `ExportVm`/`ImportVm`/`GetMigrationStatus` proto RPCs are left with a removal note for the Phase 2 proto pass.
