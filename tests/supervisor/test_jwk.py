...
def valid_jwk_headers(mocker):
    mocker.patch("aleph.vm.orchestrator.views.operator.is_token_still_valid", lambda timestamp: True)
    return {
...
async def test_expired_token(valid_jwk_headers: Dict[str, Any], mocker):
    mocker.patch("aleph.vm.orchestrator.views.operator.is_token_still_valid", lambda timestamp: False)
    request = mocker.AsyncMock()
    request.headers = valid_jwk_headers
