...
from aleph.vm.constants import GiB, MiB, Hour
...
from eth_utils import to_hex, hexstr_if_str, is_address, from_wei
...
def get_stream(sender: str, receiver: str, chain) -> Decimal:
    ...
    superfluid_instance = CFA_V1(settings.PAYMENT_RPC_SERVER, chain)
...
def compute_execution_flow_cost(execution: VmExecution) -> Decimal:
    ...
    compute_unit_price = (
            Decimal(compute_units_required) * compute_unit_multiplier * compute_unit_cost_second
    )
...
def _get_additional_storage_flow_price(execution: VmExecution) -> Decimal:
    ...
    additional_storage_second_price = Decimal(additional_storage_hour_price) / Hour
    nb_compute_units = execution.vm.hardware_resources.vcpus
    free_storage_per_compute_unit = 2 * GiB if not execution.persistent else 20 * GiB
    ...
    price = Decimal(additional_storage) / Decimal(additional_storage_second_price) / MiB
    return price
