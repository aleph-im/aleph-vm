...
# The pinned timestamp for the backup fixture (also the archive stem suffix).
BACKUP_FIXTURE_TIMESTAMP = "20260101T000000000000Z"
# A fixed mtime for the archived member files so the Python-built tar (and its
# sha256 sidecar) is byte-stable across regenerations.
BACKUP_FIXTURE_MTIME = 1_700_000_000


def build_backup_fixture() -> None:
    """Emit a backup archive built by the real qemu/backup.py code.

    Writes, under rust/crates/supervisor-daemon/tests/fixtures/backup/, a tar
    archive plus its ``.tar.sha256`` and ``.tar.meta.json`` sidecars, produced
    by ``backup._create_tar_and_checksum`` (the exact function a live node
    runs). A Rust cargo test reads these to prove the Rust reader parses the
    Python-built artifact: the member name (``rootfs.qcow2``), the sidecar
    suffixes, the meta.json field names (``vm_hash``/``created_at``/
    ``source_sizes``) and the ``{digest}  {name}`` checksum all cross-read.

    Member mtimes/modes are pinned so the archive is byte-stable when
    regenerated by the same user (tarfile records the source file's stat).
    """
    from aleph.vm.supervisor.controllers.qemu import backup as backup_module

    out_dir = FIXTURES_DIR / "backup"
    out_dir.mkdir(parents=True, exist_ok=True)
    members_dir = out_dir / "_members"
    members_dir.mkdir(exist_ok=True)
    try:
        rootfs_src = members_dir / "rootfs-src.qcow2"
        data_src = members_dir / "data-src.qcow2"
        rootfs_src.write_bytes(b"rust-fixture-rootfs-bytes")
        data_src.write_bytes(b"rust-fixture-data-bytes")
        for source in (rootfs_src, data_src):
            os.chmod(source, 0o644)
            os.utime(source, (BACKUP_FIXTURE_MTIME, BACKUP_FIXTURE_MTIME))

        tar_path = out_dir / f"{QEMU_HASH}-{BACKUP_FIXTURE_TIMESTAMP}.tar"
        backup_files = {"rootfs.qcow2": rootfs_src, "data.qcow2": data_src}
        source_sizes = {
            "rootfs.qcow2": rootfs_src.stat().st_size,
            "data.qcow2": data_src.stat().st_size,
        }
        backup_module._create_tar_and_checksum(
            tar_path,
            backup_files,
            QEMU_HASH,
            BACKUP_FIXTURE_TIMESTAMP,
            source_sizes,
        )
        print(f"wrote {tar_path} (+ .tar.sha256, .tar.meta.json)")
    finally:
        # Keep only the committed artifacts, not the transient member sources.
